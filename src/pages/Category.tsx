...
export default function Category() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await DataService.get<CategoryResponse, any>(
        `/Category/getallbypaging?page=${page}&pageSize=${pageSize}&keyword=${debouncedSearchTerm}`
      );
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      const res = await DataService.delete<any>(
        "/Category/deletemulti?checkedList=" + data
      );
      if (res) {
        if (Array.isArray(res) && res.length > 0) {
          if(res[0] == 0){
            ToastService.error("Không thể xóa danh mục vì có sản phẩm liên quan");
          }
          else{
            ToastService.success(
              `Đã xóa thành công ${selectedIds.length} danh mục`
            );
          }
        }
        fetchCategories();
    ...
    <>
              ...
              {totalPages >= 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Hiển thị trang {page + 1} trên {totalPages} - Tổng {categories.length} mục
                  </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={handleSubmitCategory}>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingCategory((prev) =>
                        prev
                          ? { ...prev, isLeaf: e.target.value === "true" }
                          : null
                      )
                    }
                  >
