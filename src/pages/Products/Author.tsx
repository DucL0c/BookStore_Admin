...
export default function Author() {
  ...
  const fetchAuthors = async () => {
    try {
      const res = await DataService.get<AuthorResponse, any>(
        `/Author/getallbypaging?page=${page}&pageSize=${pageSize}&keyword=${debouncedSearchTerm}`
      );
      if (res && Array.isArray(res.items)) {
        setAuthors(res.items);
        setTotalPages(res.totalPages);
      } else {
        ToastService.error("Dữ liệu tác giả không hợp lệ");
      }
    } catch (err) {
  ...
  const prepareEditAuthor = (author: Author) => {
  ...
  const handleAddAuthor = async () => {
    ...
    try {
      ...
      const res = await DataService.post("/Author/create", data);
      if (res) {
        ToastService.success("Thêm tác giả thành công");
        fetchAuthors();
        closeModal();
        resetForm();
      }
  ...
  const handleUpdateAuthor = async () => {
    if (!editingAuthor || editingAuthor.authorId <= 0) return;
    ...
    try {
      ...
      const res = await DataService.put("/Author/update", data);
      if (res) {
      ...
      ToastService.error("Cập nhật tác giả thất bại");
    }
  };
  ...
  const handleDeleteAuthor = async () => {
    if (selectedIds.length === 0) {
      ToastService.error("Vui lòng chọn ít nhất một tác giả để xóa");
      return;
    }

    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa ${selectedIds.length} tác giả đã chọn?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const res = await DataService.delete<any>(
        "/Author/deletemulti?checkedList=" + data
      );
      if (res) {
        if (Array.isArray(res) && res.length > 0) {
          if(res[0] == 0){
            ToastService.error("Không thể xóa tác giả vì có sản phẩm liên quan");
          }
          else{
            ToastService.success(
              `Đã xóa thành công ${selectedIds.length} tác giả`
            );
          }
        }
        fetchAuthors();
      ...
      ToastService.error("Xóa tác giả thất bại");
    }
  };
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedIds(authors.map((author) => author.authorId));
    }
    ...
    <>
          ...
          <div className="px-6 py-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
              <div className="flex items-center gap-2">
                <Button size="sm" variant="primary" onClick={prepareAddAuthor}>
                  Thêm mới
                </Button>
                <Button
                  size="sm"
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                      {authors.length > 0 ? (
                        authors.map((author, index) => (
                          <TableRow key={author.authorId ?? index}>
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <input
                                ...
                                onChange={() =>
                                  handleCheckboxChange(author.authorId)
                                }
                                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                              />
                            </TableCell>
                            ...
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <button
                                onClick={() => prepareEditAuthor(author)}
              ...
              {totalPages >= 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Hiển thị trang {page + 1} trên {totalPages} - Tổng {authors.length} mục
                  </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={handleSubmitAuthor}>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              >
                {editingAuthor?.authorId ? "Lưu thay đổi" : "Thêm mới"}
              </Button>
            </div>
          </form>
        </div>
