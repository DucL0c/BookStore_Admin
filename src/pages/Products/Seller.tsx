...
export default function Seller() {
  ...
  const fetchSellers = async () => {
    try {
      const res = await DataService.get<SellerResponse, any>(
        `/Seller/getallbypaging?page=${page}&pageSize=${pageSize}&keyword=${debouncedSearchTerm}`
      );
      if (res && Array.isArray(res.items)) {
        setSellers(res.items);
        setTotalPages(res.totalPages);
      } else {
        ToastService.error("Dữ liệu người bán không hợp lệ");
      }
    } catch (err) {
      ToastService.error("Không thể tải danh sách người bán");
    }
  };
  ...
  const prepareEditSeller = (seller: Seller) => {
  ...
  const handleAddSeller = async () => {
    ...
    try {
      const data = {
        ...
        Link: editingSeller.link,
        Logo: editingSeller.logo,
      };

      const res = await DataService.post("/Seller/create", data);
      if (res) {
        ToastService.success("Thêm người bán thành công");
        fetchSellers();
        closeModal();
        resetForm();
      }
    } catch (err) {
      ToastService.error("Thêm tác giả thất bại");
    }
  };
  ...
  const handleUpdateSeller = async () => {
    if (!editingSeller || editingSeller.sellerId <= 0) return;

    try {
      const data = {
        SellerId: editingSeller.sellerId,
        Name: editingSeller.name,
        Link: editingSeller.link,
        Logo: editingSeller.logo,
      };

      const res = await DataService.put("/Seller/update", data);
      if (res) {
        ToastService.success("Cập nhật người bán thành công");
        fetchSellers();
        closeModal();
        resetForm();
      }
    } catch (err) {
      ToastService.error("Cập nhật người bán thất bại");
    }
  };
  ...
  const handleDeleteSeller = async () => {
    if (selectedIds.length === 0) {
      ToastService.error("Vui lòng chọn ít nhất một người bán để xóa");
      return;
    }

    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa ${selectedIds.length} người bán đã chọn?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const res = await DataService.delete<any>(
        "/Seller/deletemulti?checkedList=" + data
      );
      if (res) {
        if (Array.isArray(res) && res.length > 0) {
          if(res[0] == 0){
            ToastService.error("Không thể xóa người bán vì có sản phẩm liên quan");
          }
          else{
            ToastService.success(
              `Đã xóa thành công ${res[1]} người bán`
            );
          }
        }
        fetchSellers();
      ...
      ToastService.error("Xóa người bán thất bại");
    }
  };
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedIds(sellers.map((seller) => seller.sellerId));
    }
    ...
    <>
          ...
          <div className="px-6 py-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
              <div className="flex items-center gap-2">
                <Button size="sm" variant="primary" onClick={prepareAddSeller}>
                  Thêm mới
                </Button>
                <Button
                  size="sm"
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                      {sellers.length > 0 ? (
                        sellers.map((seller, index) => (
                          <TableRow key={seller.sellerId ?? index}>
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <input
                                ...
                                onChange={() =>
                                  handleCheckboxChange(seller.sellerId)
                                }
                                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                              />
                            </TableCell>
                            ...
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <div className="w-10 h-10 overflow-hidden rounded-full">
                                <img
                                  width={40}
                                  height={40}
                                  src={seller.logo || "/images/default-avatar.png"}
                                  alt={seller.name}
                                />
                              </div>
                            </TableCell>
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <button
                                onClick={() => prepareEditSeller(seller)}
              ...
              {totalPages >= 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Hiển thị trang {page + 1} trên {totalPages} - Tổng {sellers.length} mục
                  </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={handleSubmitSeller}>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              >
                {editingSeller?.sellerId ? "Lưu thay đổi" : "Thêm mới"}
              </Button>
            </div>
          </form>
        </div>
