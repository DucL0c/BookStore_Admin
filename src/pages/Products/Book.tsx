...
export default function Book() {
  ...
  const [, setSelectedCategories] = useState<number | "">("");
  ...
  const fetchBooks = async () => {
    try {
      const res = await DataService.get<BookResponse, any>(
        `/Book/getallbypaging?page=${page}&pageSize=${pageSize}&keyword=${debouncedSearchTerm}`
      );
  ...
  const fetchCategories = async () => {
    try {
      const res = await DataService.get<Category[], any>("/Category/getall");
  ...
  const prepareAddBook = () => {
    setEditingBook({
      ...
      bookImages: [],
      bookSellers: [],
      category: { categoryId: 0, name: '', isLeaf: true }
    });
    openModal();
  };

  const prepareEditBook = (book: Book) => {
    setEditingBook({ ...book });
    openModal();
  };
  ...
  const handleAddBook = async () => {
    ...
    try {
      const data = {
        ...
        originalPrice: editingBook.originalPrice,
        listPrice: editingBook.listPrice,
        categoryId: editingBook.category.categoryId
      };

      const res = await DataService.post("/Book/create", data);
      if (res) {
  ...
  const handleUpdateBook = async () => {
    if (!editingBook || editingBook.bookId <= 0) return;
    ...
    try {
      ...
      const res = await DataService.put("/Book/Update", data);
      if (res) {
  ...
  const handleDeleteBook = async () => {
    ...
    try {
      ...
      const res = await DataService.delete<any>(
        "/Book/deletemulti?checkedList=" + data
      );
      if (res) {
          ...
          <div className="px-6 py-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
              <div className="flex items-center gap-2">
                <Button size="sm" variant="primary" onClick={prepareAddBook}>
                  Thêm mới
                </Button>
                <Button
                  size="sm"
                      ...
                      {books.length > 0 ? (
                        books.map((book, index) => (
                          <TableRow key={book.bookId ?? index}>
                            ...
                            <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              {book.listPrice.toLocaleString("vi-VN", {
                                style: "currency",
                                currency: "VND",
                              })}
                            </TableCell>
              ...
              {totalPages >= 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Hiển thị trang {page + 1} trên {totalPages} - Tổng {books.length} mục
                  </div>
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto  rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={handleSubmitBookImage}>
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  ...
                  <Select
                    ...
                    value={categories
                      ...
                      .find(option => option.value === editingBook?.category.categoryId) || null}
                    onChange={option => {
                      ...
                      if (editingBook) {
                        setEditingBook({
                          ...
                          category: {
                            ...
                            categoryId: option ? option.value : 0
                          }
                        });
                      }
                    }}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditingBook((prev) =>
                        prev ? { ...prev, originalPrice: Number(e.target.value) } : null
                      )
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    type="text"
                    value={editingBook?.listPrice || ""}
                    onChange={(e) =>
                      setEditingBook((prev) =>
                        prev ? { ...prev, listPrice: Number(e.target.value) } : null
                      )
                    }
                  />
                </div>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              >
                {editingBook?.bookId ? "Lưu thay đổi" : "Thêm mới"}
              </Button>
            </div>
          </form>
        </div>
